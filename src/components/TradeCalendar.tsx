        ...
        {(() => {
          ...
          for (let i = startingDayOfWeek; i < 7 && currentDay <= daysInMonth; i++) {
            ...
            firstWeekCells.push(
                ...
                className={`relative p-2 rounded-lg border-2 transition-all duration-200 ${
                  dayStats
                    ? dayStats.onlyScratchpad
                      ? "bg-warning/5 border-warning/30 cursor-pointer"
                      : dayStats.profit >= 0
              ...
              >
                <div className={`text-sm ${dayStats ? dayStats.onlyScratchpad ? "text-warning-foreground/60" : "text-white" : "text-foreground"}`} style={{ fontWeight: 600 }}>
                  {day}
                </div>
                {dayStats && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    {dayStats.onlyScratchpad ? (
                      <div className="text-xs text-warning-foreground/60" style={{ fontWeight: 700 }}>
                        Scratchpad
                      </div>
                    ) : (
                      <>
